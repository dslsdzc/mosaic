...
- **ServiceRegistry 泛型只增不减**:`MosaicServiceRegistry` 既有方法(register/get/optional/ref)的泛型签名**只增方法不再放宽**——放宽泛型(如把类型参数从 `Class<T>` 放宽到无界)即修改既有签名,破坏只增不减承诺;后续需要更宽泛型 → 新增方法/类型,不改既有签名。
